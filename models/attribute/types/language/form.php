...
<div id="<?php echo $htmlId ?>">
    ...
    <?php if($akcHandle == 'collection'){ ?>
        <?php echo $form->hidden($this->controller->field('value'), $value); ?> 
        <p>
            <strong><?php echo $sectionLangs[$value] ?></strong><br>
            <small><?php echo t('A page\'s language is determined by its position in the sitemap, and cannot be set manually.') ?></small>
        </p>
    <?php }else{ ?>
        <?php echo $form->select($this->controller->field('value'), array_merge(array(''=>t('Choose Language')), $sectionLangs), $value, $valueAttrs); ?> 
    <?php } ?>
    <?php if(!$isBulk && !empty($akcHandle)){ ?>
        ...
        <table style="width:100%; margin:.5em 0 0;">        
            ...
            <?php if($akcHandle == 'collection' && !Page::getByID($relation['oID']) || Page::getByID($relation['oID'])->isInTrash()) continue; ?>
